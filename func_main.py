...
def read(data, type):
    if type == 'Garmin Fenix S6':
        ...
        'MaximumRunningCadence','AverageSpeed','MaximumSpeed','TotalAscent','TotalDescent',
        'AverageStrideLength','AverageVerticalRatio','AverageVerticalOscillation',
        'AverageGroundContactTime','AveragePedalingCadence','MaximumPedalingCadence',
        'NormalizedPower_NP_','LeftRightBalance','TrainingStressScore','MaximumAveragePower_20Minutes_',
        'AveragePower','MaximumPower','Difficulty','Fluency','TotalStrokes','AverageSwolf',
        'AverageStrokeSpeed','TotalRepetition','TotalSeries','DiveTime','MinimumTemperature',
        'SurfaceInterval','Decompression','BestLapTime','NumberOfTurns','MaximumTemperature',
        'AverageRateOfRespiration','MinimumRateOfRespiration','MaximumBreathingRate',
        'ChangeInStress','BeginningOfTheStressPeriod','EndOfStressPeriod','AverageStress',
        'MovingTime','ElapsedTime','MinimumAltitude','MaximumAltitude']
        data = pd.read_csv(data, sep = ';', names = colnames, index_col=False)
        ...
        colnames=["ActivityType", "Date", "Favourite", "Title", "Distance",
                  ...
                  "AverageGroundContactTime", "TrainingStressScore",
                  "AveragePower", "MaximumPower", "Difficulty", "Fluency",
                  "AverageSwolf", "AverageStrokeSpeed",
                  "TotalRepetition", "DiveTime", "MinimumTemperature",
                  "SurfaceInterval", "Decompression", "BestLapTime", "NumberOfTurns",
                  "MaximumTemperature", "MovingTime", "ElapsedTime", "MinimumAltitude", "MaximumAltitude"]
        data = pd.read_csv(data, sep = ',', names = colnames)
    else:
        colnames=["ActivityType", "Date", "Favourite", "Title", "Distance",
                "Calories", "Time", "AverageHeartRate", "MaximumHeartRate",
                "AerobicTrainingEffect", "AverageRunningCadence",
                ...
                "TotalAscent", "TotalDescent", "AverageStrideLength",
                "AverageVerticalRatio", "AverageVerticalOscillation",
                "AverageGroundContactTime", "Avg GCT Balance",
                "TrainingStressScore",
                "AveragePower", "MaximumPower", "Difficulty", "Fluency",
                "AverageSwolf", "AverageStrokeSpeed",
                "TotalRepetition", "DiveTime", "MinimumTemperature",
                "SurfaceInterval", "Decompression", "BestLapTime", "NumberOfTurns",
                "MaximumTemperature", "Avg Resp", "Min Resp", "Max Resp", "MovingTime", "ElapsedTime", "MinimumAltitude", "MaximumAltitude"]
        data = pd.read_csv(data, sep = ',', names = colnames, index_col=False)
    return data
...
def clean(data, type):
  ...
  if type == 'Garmin Fenix S6':
      ...
      for var in to_minutes:
          data[var] = ((data[var].astype('str')).str[0:2]).astype('int')*60 + \
          ((data[var].astype('str')).str[3:5]).astype('int')
      if type == 'Garmin Forerunner':
          bound = 5
      else:
          bound = 7
      for pace in to_speed:
          cond1 = ((data[pace].str.contains(':')) & (data[pace].str.len() >= bound ))
          cond2 = ((data[pace].str.contains(':')) & (data[pace].str.len() < bound ))
          speed1 = '"'+(3600/((data.loc[cond1, pace].str[0:2]).astype(float)*60 + \
          (data.loc[cond1, pace].str[3:5]).astype(float))).astype('str')+'"'
          speed2 = '"'+(3600/((data.loc[cond2, pace].str[0:1]).astype(float)*60 + \
          (data.loc[cond2, pace].str[2:4]).astype(float))).astype('str')+'"'
      ...
      if type == 'Garmin Forerunner':
          data = data.drop(['AverageRunningCadence', 'MaximumRunningCadence',
                                ...
                                'AverageVerticalOscillation', 'AverageGroundContactTime',
                                'TrainingStressScore',
                                'MaximumPower', 'Fluency', 'AverageSwolf',
                                'AverageStrokeSpeed', 'TotalRepetition',
                                'DiveTime', 'SurfaceInterval', 'Decompression', 'BestLapTime',
                                'NumberOfTurns',  'Favourite', 'AerobicTrainingEffect'], axis=1)
      else:
          data = data.drop(['AverageRunningCadence', 'MaximumRunningCadence',
                      'AverageStrideLength', 'AverageVerticalRatio',
                      'AverageVerticalOscillation', 'AverageGroundContactTime',
                      ...
                      'AverageStrokeSpeed', 'TotalRepetition',
                      'DiveTime', 'SurfaceInterval', 'Decompression', 'BestLapTime',
                      'NumberOfTurns', 'Favourite', 'AerobicTrainingEffect',
                      'Avg Resp', 'Min Resp', 'Max Resp', 'Avg GCT Balance'], axis=1)
...
def save(model, X_train, X_test, y_train, y_test, keylist, y_pred, weekly, data, s3, bucket_name, username):
    ...
    s3.put_object(Bucket=bucket_name, Key=f"Model/{username}/y_train.npy", Body= pickle.dumps(y_train))
    s3.put_object(Bucket=bucket_name, Key=f"Model/{username}/y_test.npy", Body= pickle.dumps(y_test))
    s3.put_object(Bucket=bucket_name, Key=f"Model/{username}/y_pred.npy", Body= pickle.dumps(y_pred))

    delimiter = ""
    new_line = "\n"
    text_bytes = delimiter.join([row + new_line for row in keylist]).encode('utf-8')
